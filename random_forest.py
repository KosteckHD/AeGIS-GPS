...
def threshold_metrics_table(y_true, y_proba):
    rows = []

    for threshold in np.arange(0.05, 0.501, 0.005):
        y_pred = (y_proba >= threshold).astype(int)
        cm = confusion_matrix(y_true, y_pred, labels=[0, 1])
        tn, fp, fn, tp = cm.ravel()
...
def find_high_recall_threshold(y_true, y_proba, min_precision=0.85, min_recall=0.95):
    table = threshold_metrics_table(y_true, y_proba)
...
def main():
    ...
    high_recall_threshold, high_recall_row, val_threshold_table = find_high_recall_threshold(
        y_val,
        y_val_proba,
        min_precision=0.85,
        min_recall=0.95,
    )
    print("\nHigh-recall threshold from validation:", high_recall_threshold)
    print("High-recall validation row:")
    ...
    y_val_pred_high_recall = (y_val_proba >= high_recall_threshold).astype(int)
    ...
    print_metrics(
        f"VALIDATION high-recall threshold={high_recall_threshold:.3f}",
        y_val,
        y_val_pred_high_recall,
    )
    print_metrics(
        f"TEST high-recall threshold={high_recall_threshold:.3f}",
        y_test,
        y_test_pred_high_recall,
    )

    threshold_sweep("VALIDATION", y_val, y_val_proba)
    threshold_sweep("TEST", y_test, y_test_proba)
    val_threshold_table.to_csv("random_forest_validation_thresholds.csv", index=False)

    print("\n===== TEST METRICS BY Measurement_ID =====")
    print(metrics_by_measurement(test_df, y_test, y_test_proba, high_recall_threshold))
